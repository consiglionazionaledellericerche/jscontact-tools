...
public class ProfileTest extends JCard2JSContactTest {
    ...
    public void testRDAPProfile() throws IOException, CardException {
        ...
        assertEquals("testRDAPProfile - 18", 2, jsCard.getAddresses().size());
        assertEquals("testRDAPProfile - 19", "Suite 1234\n4321 Rue Somewhere\nQuebec\nQC\nG1V 2M2\nCanada", jsCard.getAddresses().get("int").getFullAddress());
        assertEquals("testRDAPProfile - 20", "Suite 1234", jsCard.getAddresses().get("int").getStreetExtensions());
        assertEquals("testRDAPProfile - 21", "4321 Rue Somewhere", jsCard.getAddresses().get("int").getStreetDetails());
        assertEquals("testRDAPProfile - 22", "Quebec", jsCard.getAddresses().get("int").getLocality());
        assertEquals("testRDAPProfile - 23", "QC", jsCard.getAddresses().get("int").getRegion());
        assertEquals("testRDAPProfile - 24", "Canada", jsCard.getAddresses().get("int").getCountry());
        assertEquals("testRDAPProfile - 25", "G1V 2M2", jsCard.getAddresses().get("int").getPostcode());
        assertEquals("testRDAPProfile - 26", "geo:46.772673,-71.282945", jsCard.getAddresses().get("int").getCoordinates());
        assertEquals("testRDAPProfile - 27", "Etc/GMT+5", jsCard.getAddresses().get("int").getTimeZone());
        assertEquals("testRDAPProfile - 28", "123 Maple Ave\nSuite 90001\nVancouver\nBC\n1239\n", jsCard.getAddresses().get("loc").getFullAddress());
