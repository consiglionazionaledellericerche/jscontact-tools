...
import it.cnr.iit.jscontact.tools.dto.VCardUnconvertedProperty;
 ...
 */
@NoArgsConstructor
public class VCardUnconvertedPropsDeserializer extends JsonDeserializer<VCardUnconvertedProperty[]> {
    ...
    @Override
    public VCardUnconvertedProperty[] deserialize(JsonParser jp, DeserializationContext ctxt)
            throws IOException {
        JsonNode node = jp.getCodec().readTree(jp);
        if (node == null || !node.isArray())
            return null;

        List<VCardUnconvertedProperty> list = new ArrayList<>();
        ...
        return list.toArray(new VCardUnconvertedProperty[0]);
    }
}
